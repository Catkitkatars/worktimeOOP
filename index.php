...
class OrderCounter {
    ...
    {   
        $workTimeHours = $this->workTime['finish'] - $this->workTime['start'];

        if($this->orderDate->format('H') >= $this->workTime['start'] && $this->orderDate->format('H') < $this->workTime['finish'] && $this->orderDate->format('N') != 6 && $this->orderDate->format('N') != 7) {
            $this->timer -= ($this->workTime['finish'] - $this->orderDate->format('H'));
            $this->orderDate->add(new DateInterval("P1D"));
        }
        if($this->orderDate->format('N') == 6 || $this->orderDate->format('N') == 7) {
            $this->orderDate->add(new DateInterval("P1D"));
            $this->orderDate->setTime($this->workTime['start'], 0, 0);
        }
        if($this->orderDate->format('H') >= $this->workTime['finish']) {
            $this->orderDate->add(new DateInterval("P1D"));
        }

        while($this->timer >= $workTimeHours) {

            if($this->orderDate->format('N') == 6 || $this->orderDate->format('N') == 7) {
                $this->orderDate->add(new DateInterval("P1D"));
                continue; 
            }
            $this->timer -= $workTimeHours;
            $this->orderDate->add(new DateInterval("P1D"));
        }
        ...
        while($this->orderDate->format('N') == 6 || $this->orderDate->format('N') == 7) {
            $this->orderDate->add(new DateInterval("P1D"));
        }
...
$cases = [
    'последний день февраля' => [['2023.02.28 01:23:28', [10, 19]], '2023.03.02 16:23:28'], // done
    ...
    'ошибка на единицу' => [['2023.05.15 19:00:00', [10, 19]], '2023.05.18 16:00:00'], // 
];
